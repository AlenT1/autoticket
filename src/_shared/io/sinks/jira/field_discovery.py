...
Two discovery modes — both return ``dict[str, FieldInfo]`` keyed by both
display name (lowercased) and field id (lowercased) for lookup
flexibility:

  * :func:`discover_create_meta` — uses ``/rest/api/2/issue/createmeta``;
    returns full schema (required flags + allowedValues). Needs Create
    Issues permission on the project.
  * :func:`discover_fields_from_issue` — reads an existing issue with
    ``?expand=names,schema``. Works on any read-only account but yields
    fewer details (no required flags). Useful when createmeta is
    restricted (e.g., NVIDIA's jirasw).

Then :func:`build_field_map` cross-references a user-supplied logical →
id map against the discovered fields and flags any missing entries.

Ported from ``file_to_jira.jira.field_map`` to live in ``_shared`` so
both bodies (drive + f2j) can use the same discovery surface. The only
substantive change vs the legacy version: calls go through the shared
:class:`JiraClient` directly (``client.get(path)``) instead of through
the legacy ``client._client.get(...)`` (which targeted an
...
def discover_fields_from_issue(
    ...
    Does Not Exist" if the caller lacks Create Issues permission). The
    trade-off: we can see which fields HAVE values on this ticket, but we
    don't learn `required` flags or full `allowedValues` lists. We do get
    the priority's current display value, which is usually enough to seed
    `priority_values`.
    ...
    try:
        global_fields = client.get("/field")
